...
df_test = pd.read_csv("test.csv",names=COLUMNS, skipinitialspace=True,skiprows=1,encoding='utf-8')
df_train = pd.read_csv("train.csv",names=COLUMNS, skipinitialspace=True,skiprows=1,encoding='utf-8')
...
def input_fn(df):
    ...
    feature_cols = dict(word_cols.items())#continuous_cols.items() | categorical_cols.items() |
...
t=(verified_reason_words)#[gender,follow_cnt,followers_cnt,status_cnt]+

model_dir = tempfile.mkdtemp()
m = tf.contrib.learn.LinearClassifier(feature_columns=t,
